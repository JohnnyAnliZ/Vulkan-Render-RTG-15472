...
Helpers::Allocation Helpers::allocate(VkDeviceSize size, VkDeviceSize alignment, uint32_t memory_type_index, MapFlag map){
	...
	if(map == Mapped){
		VK(vkMapMemory(rtg.device, allocation.handle, (uint32_t)allocation.offset, allocation.size, 0, &allocation.mapped));
	}
	return allocation;
}
...
Helpers::AllocatedImage Helpers::create_image(VkExtent2D const &extent, VkFormat format, VkImageTiling tiling, 
	VkImageUsageFlags usage, VkMemoryPropertyFlags properties, MapFlag map, bool is_cubemap, uint32_t mip_levels) {
...
void Helpers::transfer_to_buffer(void const *data, size_t size, AllocatedBuffer &target) {
	AllocatedBuffer transfer_src = create_buffer(size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_COHERENT_BIT|VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
	Helpers::Mapped);
	std::memcpy(transfer_src.allocation.data(), data, size);
	{//record to the cmd buffer cmd that copies the data from CPU to GPU
...
//mip map version
void Helpers::transfer_to_image(std::vector<std::vector<float>> const& mip_data, size_t size, AllocatedImage &target) {
	assert(target.handle != VK_NULL_HANDLE); //target image should be allocated already
	...
	{ // copy the source buffer to the image
		...
		for(uint32_t level = 0; level < target.mip_levels; level++){
			...
			uint32_t mip_height = target.extent.height >> level;

			size_t level_bytes = mip_data[level].size() * sizeof(float);

			size_t face_bytes = level_bytes / layer_count;
