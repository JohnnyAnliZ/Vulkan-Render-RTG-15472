...
RTG::RTG(Configuration const &configuration_) : helpers(*this) {
	...
	} else { //select the `surface_format` and `present_mode` which control how colors are represented on the surface and how new images are supplied to the surface:
	...
	{ //create the `device` (logical interface to the GPU) and the `queue`s to which we can submit commands:
		...
		if (!configuration.headless)device_extensions.emplace_back(VK_KHR_SWAPCHAIN_EXTENSION_NAME);
...
void RTG::run(Application &application) {
	...
	while (configuration.headless || !glfwWindowShouldClose(window)) {
		...
		//call render function(queue rendering work):
